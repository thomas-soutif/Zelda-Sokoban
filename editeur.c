...
debut:
...

    {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            case SDL_BUTTON_LEFT:
                if(!commandes_afficher)
                {
                    ...
                    {
                        if(event.motion.x >104 && event.motion.x < 144)
                        {
                            objetActuel = MUR;
                        }
                        if(event.motion.x >144 && event.motion.x < 184)
                        {
                            objetActuel = CAISSE;
                        }
                        if(event.motion.x >184 && event.motion.x < 224)
                        {
                            objetActuel = OBJECTIF;
                        }
                        if(event.motion.x > 224 && event.motion.x < 264)
                        {
                            objetActuel = ZELDA;
                        }
                        if(event.motion.x >264 && event.motion.x < 304)
                        {
                            objetActuel = CAISSE_OK;
                        }
                        if(event.motion.x >304 && event.motion.x < 325)
                        {
                            objetActuel = PORTE;
                        }
        ...
        if(positionCurseur.x > (Longueur) )
        {
            positionCurseur.x = (Longueur);
        }
        if(positionCurseur.y > (Largeur))
        {
            positionCurseur.y = (Largeur);
        }
        ...
        if(save == 1)
        {
            SDL_BlitSurface(imgSave,NULL,ecran, &positionSave);
        }
